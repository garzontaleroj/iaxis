...
public class Mensajes_6 extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes_6.class);
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales, new BigDecimal(6));
		return retVal;
	}
		...
		synchronized (contents_ca) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					logger.debug("Bundle Mensajes_6 actualizado");
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes_6.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes_6 no cargado", e);
			}
		}
	}
