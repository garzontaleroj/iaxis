...
public class Mensajes_9 extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes_9.class);
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales, new BigDecimal(9));
		return retVal;
	}
		...
		synchronized (contents_ca) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					logger.debug("Bundle Mensajes_9 actualizado");
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes_9.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes_9 no cargado", e);
			}
		}
	}
