...
public class Mensajes_4 extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes_4.class);
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales, new BigDecimal(4));
		return retVal;
	}
		...
		synchronized (contents_ca) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					logger.debug("Bundle Mensajes_4 actualizado");
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes_4.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes_4 no cargado", e);
			}
		}
	}
