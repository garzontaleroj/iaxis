...
public class Mensajes extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes.class);
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales,
				new BigDecimal(ConfigCache.getConfig().getProperty("locale.defecto", "1")));
		return retVal;
	}
		...
		synchronized (contents) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					contents = null;
					contents = new Object[list.size() + NUMERO_DE_LITERALES_PARAMETRIZADOS][2];
					...
					// se a�aden los literales "parametrizados"
					updateContentsParametrizados();
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes no cargado", e);
			}
		}
	}
