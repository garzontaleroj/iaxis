...
public class Mensajes_8 extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes_8.class);
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales, new BigDecimal(8));
		return retVal;
	}
		...
		synchronized (contents_ca) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					logger.debug("Bundle Mensajes_8 actualizado");
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes_8.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes_8 no cargado", e);
			}
		}
	}
