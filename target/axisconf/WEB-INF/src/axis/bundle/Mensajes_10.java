...
public class Mensajes_10 extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes_10.class);
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales, new BigDecimal(10));
		return retVal;
	}
		...
		synchronized (contents_ca) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					logger.debug("Bundle Mensajes_10 actualizado");
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes_10.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes_10 no cargado", e);
			}
		}
	}
