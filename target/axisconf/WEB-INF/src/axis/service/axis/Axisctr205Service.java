...
public class Axisctr205Service extends AxisBaseService {
	...
	public void m_siguiente(HttpServletRequest request, AbstractDispatchAction thisAction) {
		logger.debug("Axisctr205Service m_siguiente");
		Map formdata = (Map) request.getAttribute(Constantes.FORMDATA);
		...
		try {
			...
			String PTMOVIMI = getCampoTextual(request, "TMOVIMI");

			PAC_IAX_SUPLEMENTOS pac_iax_suplementos = new PAC_IAX_SUPLEMENTOS(
					(java.sql.Connection) request.getAttribute(Constantes.DB01CONN));

			Map mapa = pac_iax_suplementos.ejecutaPAC_IAX_SUPLEMENTOS__F_INSTEXMOVSEGURO(PSSEGURO, PNMOVIMI, PTMOVIMI);
			logger.debug(mapa);
			...
			// request.setAttribute("EMITIR", "EMITIR");

		} catch (Exception e) {
			logger.error("Error en el servicio Axisctr205Service - m�todo m_busqueda", e);
	...
	public void m_anterior(HttpServletRequest request, AbstractDispatchAction thisAction) {
		logger.debug("Axisctr205Service m_anterior");
		try {
			request.setAttribute(Constantes.FORWARDACTION,
					AxisCodeWizard.findForward(request, Constantes.ANTERIOR, whoami(this), null));
		} catch (Exception e) {
			logger.error("Error en el servicio Axisctr205Service - m�todo m_validar", e);
			thisAction.guardarMensaje(request, "1000050", new Object[] { e.toString() }, Constantes.MENSAJE_ERROR);
