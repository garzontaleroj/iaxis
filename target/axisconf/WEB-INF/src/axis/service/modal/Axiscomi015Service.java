...
import axis.mvc.control.modal.Axiscomi015Action;
...
/**
 * Axiscomi015Service.java 19/02/2009
 * 
 * @since Java 5.0
 */
@SuppressWarnings("all")
public class Axiscomi015Service extends AxisBaseService {
	...
	public
		...
		try {
			...
			logger.error("Error en el servicio Axiscomi015Service - m�todo m_init", e);
		} finally {
	 ...
	 * @param request
	 */
	public void m_form(HttpServletRequest request, Axiscomi015Action thisAction) {
		logger.debug("Axiscomi015Service m_form");
		UsuarioBean usuario = (UsuarioBean) request.getSession().getAttribute(Constantes.USUARIO);
		Map formdata = (Map) request.getAttribute(Constantes.FORMDATA);
		...
		try {
			...
			logger.error("Error en el servicio Axiscomi015Service - m�todo m_form", e);
	...
	private void cargarCombos(HttpServletRequest request, Axiscomi015Action thisAction) throws Exception {
		logger.debug("Axiscomi015Service m_cargarCombos");
		...
		try {
			...
			PAC_IAX_LISTVALORES pac_axis_listValores = new PAC_IAX_LISTVALORES(
					(Connection) request.getAttribute(Constantes.DB01CONN));
			// PAC_IAX_RETORNO pac_axis_Retornos = new
			// PAC_IAX_RETORNO((Connection)request.getAttribute(Constantes.DB01CONN));

			BigDecimal CEMPRES = (BigDecimal) usuario.getCempres();
			BigDecimal CRAMO = getCampoNumerico(request, "CRAMO");
			BigDecimal pidconvcomesp = getCampoNumerico(request, "PIDCONVCOMESP");
			BigDecimal pconvenio = getCampoNumerico(request, "PCONVENIO");

			ArrayList lista_productos = null;
			ArrayList lista_productos2 = (ArrayList) request.getSession().getAttribute("LSTPRODUCTOS");

			BigDecimal CRAMO_LISTA = null;
			if (!isEmpty(lista_productos2)) {
				lista_productos = lista_productos2;
				request.setAttribute("lista_productos", lista_productos);
			} else {
				if (!isEmpty(pidconvcomesp)) {
					Map m_prod = pac_iax_comisionegocio
							.ejecutaPAC_IAX_COMISIONEGOCIO__F_GET_PRODCONVCOMESP(pidconvcomesp);
					lista_productos = (ArrayList) tratarRETURNyMENSAJES(request, m_prod);
					logger.debug("********************* productos:" + m_prod);
					CRAMO_LISTA = (BigDecimal) m_prod.get("PCRAMO");
					formdata.put("CRAMO", CRAMO); // CRAMO_LISTA
					request.setAttribute("lista_productos", lista_productos);
				}

				if (!isEmpty(pconvenio)) {
					Map m_prod = pac_iax_comconvenios.ejecutaPAC_IAX_COMCONVENIOS__F_GET_PRODCONVENIO(pconvenio);
					lista_productos = (ArrayList) tratarRETURNyMENSAJES(request, m_prod);
					logger.debug("********************* productos:" + m_prod);
					CRAMO_LISTA = (BigDecimal) m_prod.get("PCRAMO");
					formdata.put("CRAMO", CRAMO); // CRAMO_LISTA
			...
			if (!isEmpty(CEMPRES)) { // Devuelve todos los ramos
				HashMap ramos = pac_axis_listValores.ejecutaPAC_IAX_LISTVALORES__F_GET_CRAMO_CONV(CEMPRES);
				...
				if (!isEmpty(lista_prod_total)) {
					...
					for (int i = 0; i < lista_prod_total.size(); i++) {
						...
						for (int j = 0; j < lista_productos.size(); j++) {
							...
							String SPRODUC_GEN = (mapa_total.get("SPRODUC")).toString();

							if (SPRODUC_GEN.equals(SPRODUC_IN.toString())) {
			...
			logger.error("Error en el m�todo Axiscomi015Service cargarCombos", e);
			thisAction.guardarMensaje(request, "1000050", new Object[] { e.toString() }, Constantes.MENSAJE_ERROR);
		} finally {
	...
	public void m_aceptar(HttpServletRequest request, Axiscomi015Action thisAction) {
		logger.debug("Axiscomi015Service m_buscar");
		UsuarioBean usuario = (UsuarioBean) request.getSession().getAttribute(Constantes.USUARIO);
		Map formdata = (Map) request.getAttribute(Constantes.FORMDATA);
		...
		try {
			...
			String LISTAPRODUCTOS = getCampoTextual(request, "LISTACONVE");
			String NULOS = getCampoTextual(request, "NULOS");

			PAC_IAX_RETORNO pac_axis_retorno = new PAC_IAX_RETORNO(
					(Connection) request.getAttribute(Constantes.DB01CONN));

			logger.debug("---> productos:" + LISTAPRODUCTOS);
			formdata.put("LISTAPRODUCTOS", LISTAPRODUCTOS);
			formdata.put("OK", "OK");
			request.getSession().setAttribute("ProductosElmimiandos", "OK");

			// TIaxInfo objeto_iax= this.generarT_IAX_Info(request, usuario, LISTACONVE, new
			// BigDecimal("1"));
			// TIaxInfo objeto_iax_nulos= this.generarT_IAX_Info(request, usuario, NULOS,
			// new BigDecimal("0"));

			/* Grabamos los campos de valor uno */
			// Map mapa_resultado =
			// pac_axis_retorno.ejecutaPAC_IAX_RETORNO__F_SET_PRODCONVENIO(IDCONVENIO,objeto_iax);
			// logger.debug(mapa_resultado);
			// BigDecimal resultado =
			// (BigDecimal)tratarRETURNyMENSAJES(request,mapa_resultado);

			/* Grabamos los campos nulos */
			/*
			 * Map mapa_resultado_nulos =
			 * pac_axis_retorno.ejecutaPAC_IAX_RETORNO__F_SET_PRODCONVENIO(IDCONVENIO,
			 * objeto_iax_nulos); logger.debug(mapa_resultado_nulos); BigDecimal
			 * resultado_nulos =
			 * (BigDecimal)tratarRETURNyMENSAJES(request,mapa_resultado_nulos);
			 * 
			 * if (resultado.intValue()==0 && resultado_nulos.intValue()==0 ){
			 * formdata.put("OK","OK"); }else{ formdata.put("OK","KO"); }
			 */
	...
	public void m_ajax_actualizar_combos(HttpServletRequest request) {
		logger.debug("Axiscomi015Service m_actualizar_combos");
		...
		try {
			...
			case 'E': // Cargar Ramos
				if (!isEmpty(CEMPRES)) {
					ramos = pacIaxListValores.ejecutaPAC_IAX_LISTVALORES__F_GET_CRAMO_CONV(CEMPRES);
			...
			logger.error("Error en el servicio Axiscomi015Service - m�todo m_actualizar_combos", e);
