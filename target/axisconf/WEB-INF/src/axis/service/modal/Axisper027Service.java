...
import axis.mvc.control.modal.Axisper027Action;
import axis.mvc.model.UsuarioBean;
import axis.service.axis.AxisBaseService;
import axis.util.Constantes;

public class Axisper027Service extends AxisBaseService {
	 ...
	 */
	public void m_form(HttpServletRequest request, Axisper027Action thisAction) {
	 ...
	 * @throws Exception
	 */
	private void cargarValoresDesplegables(HttpServletRequest request, Axisper027Action thisAction, UsuarioBean usuario,
		...
		try {
			...
			/*
			 * Map map =
			 * pac_axis_listValores.ejecutaPAC_IAX_LISTVALORES__F_GET_REGIMENFISCAL(CTIPPER)
			 * ; logger.debug(map); listValores.put("regFisc", (List)
	...
	public Persona initPersona(HttpServletRequest request, Axisper027Action thisAction) {
	 ...
	 * @throws Exception
	 */
	public BigDecimal m_grabarRegSarlaft(Persona persona, HttpServletRequest request, Axisper027Action thisAction,
			boolean isNew) {
		...
		try {
			...
			BigDecimal RES_SARLAFT = new BigDecimal("1");
			boolean todoOk = true;

			Map map = pac_axis_persona.ejecutaPAC_IAX_PERSONA__F_SET_SARLAFT(persona.SPERSON, persona.CAGENTE, FEFECTO);
			logger.debug(map);
			todoOk = new BigDecimal(0).equals(tratarRETURNyMENSAJES(request, map));

			if (todoOk) {
				RES_SARLAFT = new BigDecimal("0");
				formdata.put("RES_SARLAFT", RES_SARLAFT);
			} else
				todoOk = false;
