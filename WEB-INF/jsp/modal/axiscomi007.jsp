  ...
  <head>
        ...
        <c:if test="${__formdata.CESTADO != 1}">
         try{
           document.miForm.but_nuevo.disabled=true;   
            ...
            <c:if test="${__configform.axiscomi007__PCOMISI__modificable == 'false'}"> 
            ...
            <c:if test="${resultadoOK=='OK'}" >
                alert("<axis:alt f='axiscomi007' c='ALERT' lit='1000405' />");
                f_but_cancelar();
            </c:if>
	...
	revisarEstilos();
        ...
        function f_but_cancelar() {
            parent.f_aceptar_axiscomi007();
        }        
        
        function f_but_buscar() {
          if (objValidador.validaEntrada()) objUtiles.ejecutarFormulario("modal_axiscomi007.do", "buscar", document.miForm, "_self", objJsMessages.jslit_cargando);
        ...
        //alert("Antes de recargar");
          //aler("ACEPTAR");           
            if (objValidador.validaEntrada()) objUtiles.ejecutarFormulario("modal_axiscomi007.do", "recargar", document.miForm, "_self", objJsMessages.jslit_cargando);
        }
        
        function f_but_recargar(){
            f_cerrar_modal('axiscomi010');
        ...
        function f_aceptar_axiscomi010() {  
            f_cerrar_modal('axiscomi010');
        }
         
        function f_cancelar_axiscomi010() {
            f_cerrar_modal('axiscomi010');
        }
        ...
        function f_but_aceptar (cual) {
         ...
         function f_but_aceptar () {
            //f_but_cancelar();
            if (objValidador.validaEntrada()) objUtiles.ejecutarFormulario("modal_axiscomi007.do", "aceptar", document.miForm, "_self", objJsMessages.jslit_cargando);            
         }

         function f_cargar_valor_porcentaje(VALOR,NINIALT,NFINALT,MODALIDAD,NIVEL,SPRODUC,CACTIVI,CGARANT){
                    objAjax.invokeAsyncCGI("modal_axiscomi007.do", callbackAjaxCargarComisiones, "operation=ajax_guardar_porcentaje&PORCENTAJE=" + VALOR + 
                        "&NIVEL=" + NIVEL+"&MODALIDAD="+MODALIDAD+"&CCOMISI="+document.miForm.CCOMISI.value+"&FINIVIG="+document.miForm.FINIVIG.value+"&CGARANT="+CGARANT+"&CACTIVI="+CACTIVI+"&SPRODUC="+SPRODUC+"&FFINVIG="+document.miForm.FFINVIG.value+"&NINIALT="+NINIALT+"&NFINALT="+NFINALT, this, objJsMessages.jslit_cargando);
        }      
            ...
            function callbackAjaxCargarComisiones (ajaxResponseText){
                ...
                if(!objAppMensajes.existenErroresEnAplicacion(doc)){
                    ...
                    //objDom.addOpcionACombo("<%= Integer.MIN_VALUE %>", '- <axis:alt f="axiscomi007" c="MISS" lit="108341"/> - ', SPRODUCCombo, 0);
            ...
            function amagarDisplay(){
                try{
                    ...
                    objLista.borrarFilasDeLista ("miListaId", "5", "<axis:alt f='axiscomi007' c='MISS2' lit='1000254' />");
    ...
    //alert("hola");
    f_abrir_modal('axiscomi010',false,'&CRAMO='+document.miForm.CRAMO.value+'&CCOMISI='+document.miForm.CCOMISI.value +"&FINIVIG="+document.miForm.FINIVIG.value + '&SPRODUC='+document.miForm.SPRODUC.value + '&CACTIVI='+document.miForm.CACTIVI.value + '&CGARANT='+document.miForm.CGARANT.value);
    }
    ...
    function anyadir_fila_companyia(){    
             ...
             micompani=document.createTextNode('- ' + '<axis:alt f="axiscomi007" c="MISS" lit="108341"/>' + ' -');
            ...
            //Cargamos el campo agente en el TD de agente.
             ...
             miPE=document.createTextNode('- ' + '<axis:alt f="axiscomi007" c="MISS" lit="108341"/>' + ' -');
    ...
    //alert("En cargar combos");
       ...
       function f_cargar_productos(){
                    ...
                    objAjax.invokeAsyncCGI("modal_axiscomi007.do", callbackAjaxCargarProductos, "operation=ajax_busqueda_productos&CRAMO=" + objDom.getValorComponente(document.miForm.CRAMO) + 
        ...
        function f_cargar_actividades(){
                    ...
                    objDom.setValorComponente(document.miForm.CRAMO, null);
                    objAjax.invokeAsyncCGI("modal_axiscomi007.do", callbackAjaxCargarActividades, "operation=ajax_busqueda_actividades&CRAMO=" + objDom.getValorComponente(document.miForm.CRAMO) + 
        ...
        function callbackAjaxCargarActividades (ajaxResponseText){
                ...
                if(!objAppMensajes.existenErroresEnAplicacion(doc)){
                   ...
                   objDom.addOpcionACombo("<%= Integer.MIN_VALUE %>", '- <axis:alt f="axiscomi007" c="MISS" lit="108341"/> - ', CACTIVICombo, 0);
                   //alert("1");
                       for (i = 0; i < elementos.length; i++) {
                        //alert("2");
                            if(i== 0)
        ...
        function f_cargar_garantias(){
                    ...
                    objAjax.invokeAsyncCGI("modal_axiscomi007.do", callbackAjaxCargarGarantias, "operation=ajax_busqueda_garantias&CRAMO=" + objDom.getValorComponente(document.miForm.CRAMO) + 
        ...
        function callbackAjaxCargarGarantias (ajaxResponseText){
                ...
                if(!objAppMensajes.existenErroresEnAplicacion(doc)){
                   ...
                   objDom.addOpcionACombo("<%= Integer.MIN_VALUE %>", '- <axis:alt f="axiscomi007" c="MISS" lit="108341"/> - ', CGARANTCombo, 0);
                       for (i = 0; i < elementos.length; i++) {
       ...
       function f_abrir_axiscomi012 (CACTIVI,CGARANT,CCOMISI,CMODCOM,FINIVIG,NINIALT,SPRODUC,NFINALT,NIVEL, ESTADO) {
       ...
       function f_borrar_desglose (CACTIVI,CGARANT,CCOMISI,CMODCOM,FINIVIG,NINIALT,SPRODUC,NFINALT) {
             objUtiles.ejecutarFormulario("modal_axiscomi007.do?CACTIVISEL="+CACTIVI+"&CGARANTSEL="+CGARANT+"&CCOMISI="+CCOMISI+"&CMODCOM="+CMODCOM+"&FINIVIGSEL="+FINIVIG+"&NINIALT="+NINIALT+"&SPRODUCSEL="+SPRODUC+"&NFINALT="+NFINALT, "borrar_desglose", document.miForm, "_self", objJsMessages.jslit_cargando);            
         }
    ...
    <body class="" onload="f_onload()" >
        ...
        <c:import url="../include/modal_estandar.jsp">
            <c:param name="nid_y_titulo">axiscomi010|<axis:alt f="axiscomi007" c="TITULO" lit="9901310" /></c:param>
        </c:import>
        <form name="miForm" action="" method="POST">
          ...
          <input type="hidden" id="CONSULTA" name="CONSULTA" value="${__formdata.CONSULTA}"/>
            ...
            <c:import url="../include/titulo_nt.jsp">
                <c:param name="formulario"><axis:alt f="axiscomi007" c="FORMTIT" lit="9901311" /></c:param>
                <c:param name="producto"><axis:alt f="axiscomi007" c="FORMTIT" lit="9901311"/></c:param>
                <c:param name="form">axiscomi007</c:param>
            </c:import>
            
            <c:import url="../include/modal_estandar.jsp">
                <c:param name="titulo"><axis:alt f="axiscomi007" c="NIVELES" lit="9902456" /></c:param>
                <c:param name="nid" value="axiscomi011" />
            </c:import>
            
            <c:import url="../include/modal_estandar.jsp">
                <c:param name="titulo"><axis:alt f="axiscomi007" c="DESGLOSE" lit="9904658" /></c:param>
                ...
                <tr>
                    <td>
                        ...
                        <table class="seccion" id="LISTA">
                            ...
                            <tr>
                                <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="CCOMISI" lit="109155" /></b><!-- Código -->
                                </td>
                                <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="TCOMISI" lit="100588" /></b><!-- Descripcion -->
                                </td>
                                <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="FINIVIG" lit="9000526" /></b><!-- Fecha inicio -->
                                </td>
                                <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="FFINVIG" lit="9000527" /></b><!-- Fecha fin -->
                            ...
                            <tr>
                                 <td class="campocaja">
                                    <input type="text" readonly class="campowidthinput campo campotexto" title="<axis:alt f="axiscomi007" c="CCOMISI" lit="109155"/>" size="15" value="${__formdata['CCOMISI']}" name="CCOMISI" id="CCOMISI"/>
                                </td>
                                <td class="campocaja">
                                    <input type="text" readonly class="campowidthinput campo campotexto" title="<axis:alt f="axiscomi007" c="TCOMISI" lit="100588"/>" value="${__formdata['TCOMISI']}" name="TCOMISI" id="TCOMISI" size="15"/>
                                </td>
                                <td class="campocaja">
                                    <input type="text" readonly  class="campowidthinput campo campotexto" style="width:50%" value="<fmt:formatDate pattern="dd/MM/yyyy" value="${__formdata.FINIVIG}"/>" name="FINIVIG" id="FINIVIG" size="15"
                                     title="<axis:alt f="axiscomi007" c="FINIVIG" lit="9000526"/>"/>
                                </td>      
                                <td class="titulocaja">
                                    <input type="text"  readonly class="campowidthinput campo campotexto" style="width:50%" value="<fmt:formatDate pattern="dd/MM/yyyy" value="${__formdata.FFINVIG}"/>" name="FFINVIG" id="FFINVIGFFINVIG" size="15"
                                     title="<axis:alt f="axiscomi007" c="FFINVIG" lit="9000527"/>"/>
                                </td>
                            </tr>
                        </table>
                        ...
                        <table class="seccion">
                            ...
                            <tr>
                                ...
                                <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="CATRIBU" lit="111471" /></b> <!-- Agrupacion -->
                                </td>
                                <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="CRAMO" lit="100784" /></b> <!-- Ramo -->
                                </td>
                                 <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="SPRODUC" lit="100829" /></b> <!-- Producto -->
                                </td>
                            ...
                            <tr>                         
                                    <td class="campocaja">
                                        <select name = "CAGRUPA" id="CAGRUPA" style="width:200px;"  size="1" onchange="f_but_buscar();" class="campowidthselect campo campotexo">
                                            <option value = "<%= Integer.MIN_VALUE %>"> - <axis:alt f="axiscomi007" c="MIN_VALUE" lit="108341"/> - </option>
                            ...
                            <tr>
                                ...
                                <td class="titulocaja" >
                                    <b><axis:alt f="axiscomi007" c="CACTIVI" lit="103481" /></b> <!-- Actividad -->
                                </td>
                                <td class="titulocaja">
                                    <b><axis:alt f="axiscomi007" c="CGARANT" lit="110994" /></b> <!-- garantia -->
                                </td>
                                 <td class="titulocaja">
                            ...
                            <!--tr>
                                <td align="right">
                                    <input align="right" type="button" class="boton" id="but_buscar" value="<axis:alt f="axiscomi007" c="BUSCAR" lit="100797" />" onclick="f_but_buscar()" />
                                </td>
                            </tr-->
                            <tr>
                    ...
                    <td>
                        ...
                        <!--div style="align:right;">
                              <a href="javascript:f_abrir_modal('axiscomi010');"><img border="0" alt="<axis:alt f="axiscomi007" c="NEW" lit="104825"/>" title="<axis:alt f="axiscomi007" c="NEW" lit="104825"/>" src="images/new.gif"/></a>
                        </div--> 
                        <div class="separador">&nbsp;</div>
                            <!-- DisplayTag Pólizas -->
                            <c:set var="title1"><axis:alt f="axiscomi007" c="SPRODUC" lit="100829"/></c:set><!-- Producto -->
                            <c:set var="title2"><axis:alt f="axiscomi007" c="TACTIVI" lit="103481"/></c:set><!-- Actividad -->
                            <c:set var="title3"><axis:alt f="axiscomi007" c="CGARANT" lit="110994"/></c:set><!--Garantia -->
                            <c:set var="title4"><axis:alt f="axiscomi007" c="TMODCOM" lit="100943"/></c:set><!-- Modalidad -->
                            <c:set var="title5"><axis:alt f="axiscomi007" c="PRODLIST" lit="101467"/></c:set><!-- Procentaje -->
                            <c:set var="title9"><axis:alt f="axiscomi007" c="NINIALT" lit="104095"/></c:set><!--NINIALT-->
                            <c:set var="title10"><axis:alt f="axiscomi007" c="NFINALT" lit="103051"/></c:set><!--NFINALT-->
                            <!-- TODOC: nuevos campos MSV. Literales-->
                            <axis:visible f="axiscomi007" c="CCRITERIO">
                                <c:set var="title11"><axis:alt f="axiscomi007" c="NDESDE" lit="104095"/>(<axis:alt f="axiscomi007" c="CCRITERIO" lit="9901472"/>)</c:set><!--NDESDE-->
                                <c:set var="title12"><axis:alt f="axiscomi007" c="NHASTA" lit="103051"/>(<axis:alt f="axiscomi007" c="CCRITERIO" lit="9901472"/>)</c:set><!--NHASTA-->
                                <c:set var="title13"><axis:alt f="axiscomi007" c="CCRITERIO" lit="9901472"/></c:set><!--CCRITERIO-->
                            </axis:visible>
                            <!-- TODOC: Fin nuevos campos MSV. Literales-->
                            <div class="seccion displayspaceGrande">
                                <display:table name="${sessionScope.axisctr_listaComisiones}" id="LISTA2" export="false" class="dsptgtable" pagesize="-1" requestURI="modal_axiscomi007.do?paginar=true" sort="list" cellpadding="0" cellspacing="0">
                                    ...
                                    <display:column title="${title1}" sortable="true" sortProperty="SPRODUCLIST" headerClass="headwidth10 sortable"  media="html" autolink="false" >    
                                         <div class="dspText">${LISTA2.OB_IAX_DETCOMISION.TTITULO}</div>
                                    </display:column>
                                    <display:column title="${title2}" sortable="true" sortProperty="TACTIVILIST" headerClass="sortable" media="html" autolink="false" >
                                        <div class="dspText">${LISTA2.OB_IAX_DETCOMISION.TACTIVI}</div>
                                    </display:column>
                                    <display:column title="${title3}" sortable="true" sortProperty="CGARANTLIST" headerClass="sortable" media="html" autolink="false" >
                                        <div class="dspText">${LISTA2.OB_IAX_DETCOMISION.CGARANT} - ${LISTA2.OB_IAX_DETCOMISION.TGARANT}</div>
                                    </display:column>
                                    <display:column title="${title4}" sortable="true" sortProperty="CMODCOMLIST" headerClass="sortable" media="html" autolink="false" >
                                        <div class="dspText">${LISTA2.OB_IAX_DETCOMISION.TMODCOM}</div>
                                    </display:column>
                                    
                                    
                                    <c:if test="${__configform.axiscomi007__PCOMISI__modificable == 'false' || !empty __formdata.CONSULTA || LISTA2.OB_IAX_DETCOMISION.PDESGLOSE == 1 }">
                                        <display:column title="${title5}" sortable="true" sortProperty="DESPRODUCTOLIST" headerClass="sortable" media="html" autolink="false" >
                                            <div class="dspText">
                                                <fmt:formatNumber value="${LISTA2.OB_IAX_DETCOMISION.PCOMISI}" pattern="###,##0.00"/>
                                            </div>
                                        </display:column>
                                    </c:if>
                                    <c:if test="${__configform.axiscomi007__PCOMISI__modificable != 'false' && empty __formdata.CONSULTA }">
                                       <c:if test="${LISTA2.OB_IAX_DETCOMISION.PDESGLOSE == 0}">
                                        <display:column title="${title5}" sortable="true" sortProperty="DESPRODUCTOLIST" headerClass="sortable" media="html" autolink="false" >
                                            <div class="dspText"><input type="text" class="campowidthinput campo campotexto" id="CAMPO" value="<fmt:formatNumber value="${LISTA2.OB_IAX_DETCOMISION.PCOMISI}" pattern="###,##0.00"/>" onchange="javascript:f_cargar_valor_porcentaje(this.value,'${LISTA2.OB_IAX_DETCOMISION.NINIALT}','${LISTA2.OB_IAX_DETCOMISION.NFINALT}','${LISTA2.OB_IAX_DETCOMISION.CMODCOM}','${LISTA2.OB_IAX_DETCOMISION.NIVEL}',${LISTA2.OB_IAX_DETCOMISION.SPRODUC},'${LISTA2.OB_IAX_DETCOMISION.CACTIVI}','${LISTA2.OB_IAX_DETCOMISION.CGARANT}')"></input></div>
                                        </display:column>
                                       </c:if> 
                                    </c:if>
                                    <!-- TODOC: nuevos campos MSV-->
                                   <axis:visible f="axiscomi007" c="CCRITERIO">
                                        <display:column title="${title13}" sortable="true" sortProperty="TCRITERIO" headerClass="sortable fixed" media="html" autolink="false" >
                                           <div class="dspText">${LISTA2.OB_IAX_DETCOMISION.TCRITERIO}</div>
                                        </display:column>
                                   </axis:visible>
                                   
                                   <axis:visible f="axiscomi007" c="NDESDE">
                                        <display:column title="${title11}" sortable="true" sortProperty="NDESDE" headerClass="sortable fixed" media="html" autolink="false" >
                                            <div class="dspText">
                                                <fmt:formatNumber value="${LISTA2.OB_IAX_DETCOMISION.NDESDE}" pattern="##0"/>
                                            </div>
                                        </display:column>
                                    </axis:visible>
                                   
                                    <axis:visible f="axiscomi007" c="NHASTA">
                                        <display:column title="${title12}" sortable="true" sortProperty="NHASTA" headerClass="sortable fixed" media="html" autolink="false" >
                                            <div class="dspText">
                                                <fmt:formatNumber value="${LISTA2.OB_IAX_DETCOMISION.NHASTA}" pattern="##0"/>
                                            </div>
                                        </display:column>
                                    </axis:visible>
                                    <!-- TODOC: nuevos campos MSV-->
                                    <axis:visible f="axiscomi007" c="NINIALT">
                                        <display:column title="${title9}" sortable="true" sortProperty="NINIALT" headerClass="sortable fixed" media="html" autolink="false" >
                                            <div class="dspText">
                                                <fmt:formatNumber value="${LISTA2.OB_IAX_DETCOMISION.NINIALT}" pattern="##0"/>
                                            </div>
                                        </display:column>
                                    </axis:visible>
                                    
                                    <axis:visible f="axiscomi007" c="NFINALT">
                                        <display:column title="${title10}" sortable="true" sortProperty="NFINALT" headerClass="sortable fixed" media="html" autolink="false" >
                                            <div class="dspText">
                                                <fmt:formatNumber value="${LISTA2.OB_IAX_DETCOMISION.NFINALT}" pattern="##0"/>
                                            </div>
                                        </display:column>
                                    </axis:visible>
                                      <c:if test="${__formdata.CESTADO != 2 && empty __formdata.CONSULTA}">
                                    <axis:visible f="axiscomi007" c="BUT_NIVELES">
                                        <display:column title="" sortable="false" media="html" autolink="false" headerClass="sortable" >
                                            <div class="dspIcons">
                                                <img border="0" alt="<axis:alt f="axiscomi007" c="NIVELES" lit="9902456"/>" title1="<axis:alt f="axiscomi007" c="NIVELES" lit="9902456"/>" src="images/lapiz.gif" 
                                                 style="cursor:pointer;" onclick="javascript:f_abrir_axiscomi011 (${LISTA2.OB_IAX_DETCOMISION.SPRODUC},'${LISTA2.OB_IAX_DETCOMISION.CACTIVI}','${LISTA2.OB_IAX_DETCOMISION.CGARANT}','${LISTA2.OB_IAX_DETCOMISION.CCOMISI}','${LISTA2.OB_IAX_DETCOMISION.CMODCOM}','<fmt:formatDate pattern="dd/MM/yyyy" value="${LISTA2.OB_IAX_DETCOMISION.FINIVIG}"/>')"/>
                                            </div>
                                        </display:column>
                                    </axis:visible>
                                    </c:if>
                                    <c:if test="${__formdata.CESTADO != 3 && empty __formdata.CONSULTA}">
                                    <axis:visible f="axiscomi007" c="BUT_DESGLOSE">
                                        <display:column title="" sortable="false" media="html" autolink="false" headerClass="sortable" >
                                            <div class="dspIcons">
                                                <img border="0" alt="<axis:alt f="axiscomi007" c="DESGLOSE" lit="9904656"/>" title1="<axis:alt f="axiscomi007" c="DESGLOSE" lit="9904656"/>" src="images/lapiz.gif" 
                                                 style="cursor:pointer;" onclick="javascript:f_abrir_axiscomi012 ('${LISTA2.OB_IAX_DETCOMISION.CACTIVI}','${LISTA2.OB_IAX_DETCOMISION.CGARANT}','${LISTA2.OB_IAX_DETCOMISION.CCOMISI}','${LISTA2.OB_IAX_DETCOMISION.CMODCOM}','<fmt:formatDate pattern="dd/MM/yyyy" value="${LISTA2.OB_IAX_DETCOMISION.FINIVIG}"/>','${LISTA2.OB_IAX_DETCOMISION.NINIALT}','${LISTA2.OB_IAX_DETCOMISION.SPRODUC}','${LISTA2.OB_IAX_DETCOMISION.NFINALT}','${LISTA2.OB_IAX_DETCOMISION.NIVEL}', '${__formdata.CESTADO}')"/>
                                             </div>
                                        </display:column>
                                    </axis:visible>
                                    </c:if>
                                    <c:if test="${__formdata.CESTADO != 2 && empty __formdata.CONSULTA}">
                                    <axis:visible f="axiscomi007" c="BUT_DELDESGLOSE">
                                        <display:column title="" sortable="false" media="html" autolink="false" headerClass="sortable" >
                                            <div class="dspIcons">
                                                <img border="0" alt="<axis:alt f="axiscomi007" c="DELDESGLOSE" lit="9904657"/>" title1="<axis:alt f="axiscomi007" c="DESGLOSE" lit="9904657"/>" src="images/delete.gif" 
                                                 style="cursor:pointer;" onclick="javascript:f_borrar_desglose('${LISTA2.OB_IAX_DETCOMISION.CACTIVI}','${LISTA2.OB_IAX_DETCOMISION.CGARANT}','${LISTA2.OB_IAX_DETCOMISION.CCOMISI}','${LISTA2.OB_IAX_DETCOMISION.CMODCOM}','<fmt:formatDate pattern="dd/MM/yyyy" value="${LISTA2.OB_IAX_DETCOMISION.FINIVIG}"/>','${LISTA2.OB_IAX_DETCOMISION.NINIALT}','${LISTA2.OB_IAX_DETCOMISION.SPRODUC}','${LISTA2.OB_IAX_DETCOMISION.NFINALT}')"/>
                                             </div>
                                        </display:column>
                                    </axis:visible>
                                     </c:if>
...
<c:choose>                        
            ...
            <c:import url="../include/botonera_nt.jsp">
                <c:param name="f">axiscomi007</c:param><c:param name="__botones">cancelar<axis:visible c="BT_NUEVO" f="axiscomi007">,nuevo</axis:visible>,aceptar</c:param>
            </c:import>
            </c:when>
            <c:otherwise>
             <c:import url="../include/botonera_nt.jsp">
            <c:param name="f">axiscomi007</c:param><c:param name="__botones">cancelar</c:param>
             </c:import>
            </c:otherwise>
            </c:choose>
