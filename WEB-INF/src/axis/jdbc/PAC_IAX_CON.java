...
public class PAC_IAX_CON extends AccesoPL {
	...
	private HashMap callPAC_IAX_CON__F_ANTIGUEDADCONDUCTOR(String pPTIPODOC, String pPNNUMIDE,
			java.math.BigDecimal pPSINTERF) throws Exception {
		String callQuery = "{?=call PAC_IAX_CON.F_ANTIGUEDADCONDUCTOR(?, ?, ?, ?, ?, ?, ?)}";

		logCall(callQuery, new String[] { "pPTIPODOC", "pPNNUMIDE", "pPSINTERF" },
				new Object[] { pPTIPODOC, pPNNUMIDE, pPSINTERF });
		CallableStatement cStmt = conn.prepareCall(callQuery);
		String USERNAME = conn.getMetaData().getUserName().toUpperCase();
	...
	public HashMap ejecutaPAC_IAX_CON__F_ANTIGUEDADCONDUCTOR(String pPTIPODOC, String pPNNUMIDE,
			java.math.BigDecimal pPSINTERF) throws Exception {
	...
	// --START-PAC_IAX_CON.F_ULTIMOSEGUROVEHICULO(PTIPOMATRICULA, PMATRICULA,
	// PSINTERF)
	private HashMap callPAC_IAX_CON__F_ULTIMOSEGUROVEHICULO(String pPTIPOMATRICULA, String pPMATRICULA,
			java.math.BigDecimal pPSINTERF) throws Exception {
		String callQuery = "{?=call PAC_IAX_CON.F_ULTIMOSEGUROVEHICULO(?, ?, ?, ?, ?, ?)}";

		logCall(callQuery, new String[] { "pPTIPOMATRICULA", "pPMATRICULA", "pPSINTERF" },
				new Object[] { pPTIPOMATRICULA, pPMATRICULA, pPSINTERF });
		CallableStatement cStmt = conn.prepareCall(callQuery);
		String USERNAME = conn.getMetaData().getUserName().toUpperCase();
	...
	public HashMap ejecutaPAC_IAX_CON__F_ULTIMOSEGUROVEHICULO(String pPTIPOMATRICULA, String pPMATRICULA,
			java.math.BigDecimal pPSINTERF) throws Exception {
	...
	// --START-PAC_IAX_CON.F_LISTA_CONTRATOS_PAGO(PIPAGO, PSPERSON, PNSINIES,
	// PSSEGURO)
	private HashMap callPAC_IAX_CON__F_LISTA_CONTRATOS_PAGO(java.math.BigDecimal pPIPAGO,
			java.math.BigDecimal pPSPERSON, String pPNSINIES, java.math.BigDecimal pPSSEGURO) throws Exception {
		String callQuery = "{?=call PAC_IAX_CON.F_LISTA_CONTRATOS_PAGO(?, ?, ?, ?, ?, ?, ?)}";

		logCall(callQuery, new String[] { "pPIPAGO", "pPSPERSON", "pPNSINIES", "pPSSEGURO" },
				new Object[] { pPIPAGO, pPSPERSON, pPNSINIES, pPSSEGURO });
		CallableStatement cStmt = conn.prepareCall(callQuery);
		String USERNAME = conn.getMetaData().getUserName().toUpperCase();
		...
		cStmt.registerOutParameter(6, java.sql.Types.NUMERIC); // Valor de "PSINTERF"
		cStmt.registerOutParameter(7, oracle.jdbc.OracleTypes.ARRAY,
				UsuarioBean.fixOwner(USERNAME, "T_IAX_SIN_TRAMI_PAGO_CTR".toUpperCase())); // Valor de "PCURCONTRATOS"
		cStmt.registerOutParameter(8, oracle.jdbc.OracleTypes.ARRAY,
				UsuarioBean.fixOwner(USERNAME, "T_IAX_MENSAJES".toUpperCase())); // Valor de "MENSAJES"
		cStmt.execute();
		HashMap retVal = new HashMap();
	...
	public HashMap ejecutaPAC_IAX_CON__F_LISTA_CONTRATOS_PAGO(java.math.BigDecimal pPIPAGO,
			java.math.BigDecimal pPSPERSON, String pPNSINIES, java.math.BigDecimal pPSSEGURO) throws Exception {
