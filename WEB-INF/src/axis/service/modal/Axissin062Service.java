...
import axis.mvc.control.modal.Axissin062Action;
import axis.mvc.model.UsuarioBean;
import axis.service.axis.Axissin006Service;
import axis.util.Constantes;

public class Axissin062Service extends Axissin006Service {
	 ...
	 * @param thisAction
	 */
	public void m_form(HttpServletRequest request, Axissin062Action thisAction) {
	...
	public void m_recargar(HttpServletRequest request, Axissin062Action thisAction) {
		...
		try {
			...
			logger.error("Error en el servicio Axissin062Service - m�todo m_recargar", e);
			thisAction.guardarMensaje(request, "1000052", new Object[] { e.toString() }, Constantes.MENSAJE_ERROR);
		}
		this.f_Cargar_Propiedades_Pantalla(request, this.whoami(this));
	}

	public void genera_docupendiente(HttpServletRequest request, Axissin062Action thisAction) {
		logger.debug("Axissin062Service m_recargar");
		UsuarioBean usuario = (UsuarioBean) request.getSession().getAttribute(Constantes.USUARIO);
		Map formdata = (Map) request.getAttribute(Constantes.FORMDATA);
			...
			if (pRETURN != null && pRETURN.intValue() != 1) { /////////// AVISOS
				...
				if (resultado.intValue() == 0 || pRETURN1.intValue() == 0) {
					mapa = (Map) pac_axis_siniestros.ejecutaPAC_IAX_SINIESTROS__F_SET_CAMPO_PLANTILLA(NSINIES,
							SINIESDOCPEN, SPRODUC, null, CCAMPO, DOCUPEN);
