...
import axis.mvc.control.axis.Axismpr006Action;
import axis.mvc.model.UsuarioBean;
import axis.util.Constantes;

public class Axismpr006Service extends AxismprxxxService {
	...
	public void m_form(HttpServletRequest request, Axismpr006Action thisAction) {
		...
		try {
			...
			logger.error("Error en el servicio Axismpr006Service - m�todo m_form", e);
			AbstractDispatchAction.guardarMensaje(request, "1000050", new Object[] { e.toString() },
					Constantes.MENSAJE_ERROR);
		}
	...
	public void m_borrar(HttpServletRequest request, Axismpr006Action thisAction) {
		UsuarioBean usuario = (UsuarioBean) request.getSession().getAttribute(Constantes.USUARIO);
		Map formdata = (Map) request.getAttribute(Constantes.FORMDATA);
	...
	public void m_actualizar(HttpServletRequest request, Axismpr006Action thisAction) {
		UsuarioBean usuario = (UsuarioBean) request.getSession().getAttribute(Constantes.USUARIO);
		Map formdata = (Map) request.getAttribute(Constantes.FORMDATA);
