...
public class Mensajes_3 extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes_3.class);
	private static Properties literales = new Properties();
	static Object[][] contents_3 = new Object[1][2];
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales, new BigDecimal(3));
		return retVal;
	}
		...
		synchronized (contents_3) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					contents_3 = new Object[list.size()][2];
					Map m = null;
					for (int i = 0; i < list.size(); i++) {
						m = (Map) list.get(i);
						contents_3[i][0] = m.get("ALIAS");
						contents_3[i][1] = m.get("LITERAL");
					}

					logger.debug("Bundle Mensajes_3 actualizado");
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes_3.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes_3 no cargado", e);
			}
		}
	}
