...
public class Mensajes_5 extends ListResourceBundle {
	private static Log logger = LogFactory.getLog(Mensajes_5.class);
	...
	private static List loadContents() {
		...
		retVal = MensajesUtil.loadContens(literales, new BigDecimal(5));
		return retVal;
	}
		...
		synchronized (contents_ca) {
			try {
				...
				if (!AxisBaseService.isEmpty(list)) {
					...
					logger.debug("Bundle Mensajes_5 actualizado");
				} else
					throw new Exception("ERROR. No se han cargado literales para Mensajes_5.");
			} catch (Exception e) {
				logger.fatal("Bundle Mensajes_5 no cargado", e);
			}
		}
	}
