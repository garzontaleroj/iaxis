...
public class XML2ObjectFactory extends DefaultHandler {
	static Log logger = LogFactory.getLog(XML2ObjectFactory.class);
	...
	public static void main(String argv[]) {
		...
		logger.debug("Objeto creado a partir de XML " + test_XML_FILE_NAME + ":\n");
		if (object != null) {
			logger.debug("Tipo del objeto: " + object.getClass().getName() + "\n");
			logger.debug(object);
		}
	...
	/**
	 * Dado un String conteniendo c�digo XML v�lido, devuelve el objeto resultado de
			...
			if (currentNodeClass != null) {
				if (listOfParents.isEmpty()) {
					...
					if (currentNodeClass.getName().indexOf("java.util.") >= 0) {
						...
						// A�adir a una tabla adicional, para saber que no tenemos que tratar su
						// "endElement"
						...
						Object currentObject = instantiate(currentNodeClass,
								isEmpty(currentNodeValue) ? null : currentNodeValue);
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		else if (qName != rootNodeName && currentNodeClass != null
				&& currentNodeClass.getName().indexOf("java.util.") < 0) {
