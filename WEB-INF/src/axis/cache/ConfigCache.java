...
public class ConfigCache {
	static Log logger = LogFactory.getLog(ConfigCache.class);
	...
	private ConfigCache() {
		try {
			...
			logger.error("Inicializacion de " + MINOMBRE + ".", e);
		}
	}
	...
	public static HashMap getData() throws Exception {
		if (lastUpdate == null || (lastUpdate.getTime() + ConfigCache.MAXCACHEDTIMEINMILLIS) < (new Date()).getTime()) {
			reload();
		}
		return data;
	}
	...
	public static void setMAXCACHEDTIMEINMILLIS(long p) {
		...
		if (MAXCACHEDTIMEINMILLIS < 0)
			MAXCACHEDTIMEINMILLIS = defaultMAXCACHEDTIMEINMILLIS;
	}
	...
	public static void main(String[] args) throws Exception {
		logger.debug(ConfigCache.getConfig());
